...
foreach my $pdfn (readdir(LWD)) {
	my @ypages = (
		'1|22.5|15.5|1|02_2,2_01_1.png',
		'1|22.5|13|1|02_4,3_01_0.png',
		'1|1.5|1|1.5|redbook.png',
	);

	$pdfn = decode('utf-8', $pdfn);
	next if(-d $pdfn or $pdfn !~ m/pdf$/);
	print "$pdfn\n";

	my ($fid, $fno, $tid, $tno, $bid);
	if($pdfn =~ m/(\d+)至(\d+)/) {
		($fid, $tid) = ($1, $2);
		$fid-- if($fid > 1);
		$tid--;
	...
	my $epdfn = "$bid、《$book{'title'}》卷$fno\至$tno".'.pdf';
	...
	foreach my $pid (@fpages) {
		push @ypages, "$pid|0.95|8|1|02_2,2_01_1.png";
		push @ypages, "$pid|0.95|6|1|02_1,2_01_0.png";
	}
