...
if(not -d "books_mr/$book_id") { print "错误：未发现该书籍目录'books_mr/$book_id'！\n"; exit; }
if(not -d "books_mr/$book_id/text" ) { print "错误: 未发现该书籍文本目录'books_mr/$book_id/text'！\n"; exit; }
if(not -f "books_mr/$book_id/book.cfg") { print "错误：未发现该书籍排版配置文件'books_mr/$book_id/book.cfg'！\n"; exit; }
...
#读取书籍配置文件
my %book;
open BCONFIG, "< books_mr/$book_id/book.cfg";
print "读取书籍排版配置文件'books_mr/$book_id/book.cfg'...\n";
while(<BCONFIG>) {
	chomp;
	next if(m/^\s{0,}$/);
	next if(m/^#/);
...
my @dats = ('');
#读取书籍文本
opendir TDIR, "books_mr/$book_id/text";
print "读取该书籍全部文本文件'books_mr/$book_id/text/*.txt'...";
foreach my $tfn (sort readdir(TDIR)) {
	...
	#读取文件，计算段落首尾需要补齐的空格
	my $dat;
	my $text_file = "books_mr/$book_id/text/$tfn";
	open TEXT, "< $text_file";
	while(<TEXT>) {
		chomp;
		next if(/^\s{0,}$/);
...
#添加封面，封面图片不存在时添加简易封面
if(-f "books_mr/$book_id/cover.jpg") {
	print "发现封面图片'books_mr/$book_id/cover.jpg ...\n";
	my $cpimg = $vpdf->image("books_mr/$book_id/cover.jpg");
	$vpage->object($cpimg);
} else {
	print "未发现封面文件'books_mr/$book_id/cover.jpg，创建简易封面...\n";
	my $pline = $vpage->gfx();
...
foreach my $tid ($from..$to) {
	...
	print "读取'books_mr/$book_id/text/'目录下第 $tid "."个文本文件...\n";
	my $dat = $dats[$tid];
	my @chars = split //, $dat;
...
$vpdf->save("books_mr/$book_id/$pdfn.pdf");
print "完成！\n";

if(defined $opts{'c'}) {
	my $input = "books_mr/$book_id/$pdfn.pdf";
	my $output = "books_mr/$book_id/$pdfn".'_已压缩.pdf';
